...
| `openarch` <https://www.openarchieven.nl/> | index | open | api, name-index | 68 | About 30 million Belgian person-mentions: the Familiekunde Vlaanderen and Doodsprentjes.be bidprentjes and rouwbrieven, the heemkring collections, and the Rijksarchief civil acts transcribed by the Demogen volunteers. Coverage is uneven by province — Vlaams-Brabant has indexed civil acts with full parent roles; Oostende and Evergem are overwhelmingly 20th-century memorial cards. |
...
### FamilySearch full-text search (machine-read unindexed images)

#### `fs-fulltext-bundervoet` — FamilySearch full-text search - Bundervoet, worldwide, exhaustive
- **Kind:** index · <https://www.familysearch.org/nl/search/full-text/results?q.text=Bundervoet>
- **Covers:** ['Bundervoet']
- **Yielded:** 237 documents, paged to exhaustion. 135 are GENT NOTARIAL records 1659-1757 - a class of act no other venue in this registry indexes, and the one that names family relationships outright. Also 9 Rotterdam guardianship accounts that confirm the 17th-century Michiel Geertssen family from primary sources, and Kortrijk citizenship rolls of the 1540s naming Bundervoets with patronymics, the earliest occurrence of the surname yet found anywhere.
- **Confidence:** sup
- **Accessed:** 2026-07-27
- **Note:** NOT blocked, and research-log SS76 was wrong to record it so. The failure is a bug in FamilySearch's own UI: it sends q.textCountry, the API answers 400 'Unable to map supplied value=text_country to query term', and the page renders 'Er is iets misgegaan'. Drop the country filter and it works - so any country-filtered full-text search appears broken to every user. Query the service endpoint directly and page with count/offset. The text is handwriting-recognition output and garbles names badly; treat it as a FINDING AID saying which image to read, never as a transcription.
