...
| — | 2026-07-27 | doc | sessions restored: emma_vincke + 1907 legitimation | GRAFTED | — | — |
| — | 2026-07-27 | doc | kraainem 1884/1887 images | GRAFTED | franciscus_swaelens | — |
...
|---|---|---|---|
| 1 | `schalandrijn_octavia_1886` + `devriendt_eugenius_1882` parents — 4 people, **doc** | 1906 Oostende marriage act **image**: groom b. Steene 13 Apr 1882, to the day | Bredene birth act image: *"geboren eergisteren de dertigsten April"* |
| 2 | `bossin_anna_1829` + Pardon parents — 4 people, sup | parents named identically to her brother's 1846 act | her brother **Guilielmus Bossin, 29**, a witness at the wedding |
| 3 | `coekelberghs_maria_1848` + `vandenbemden_henricus_1849` parents — 5 people, sup | marriage date 24 Apr 1873 | groom's birth, Everberg 19 Aug 1849, to the day |
| 5 | `coekelberghs_joannes_1809` + `haesaerts_anna_1810` parents — 4 people, sup | 1838 Bertem marriage act **image**, spousal pair | father self-declaring aged 44 (1853) and 48 (1858), both consistent with b. Jul 1809 |
...
| 7 | `bocklandt_louise_1877` restored to **doc**, + 10 people | 1924 Stene marriage act **image**: b. 31 Dec 1877 Hamme, to the day | the De Keyser marriage *and* divorce |
| 9 | `vandenberghe_paulina_1849` + `vermorgen_petrus`, **doc** | 1882 Hamme act **image** dates the mother's death 21 Jun 1865 Hamme | father's trade *metser*, matching two other acts |
...
- **`coekelberghs_joannes_1858` — mother link removed.** The act says *Anna **Catharina***
...
- `schalandrijn_octavia_1886` b. 1886-05-02 → **1886-04-30** (the first was the act date).
- `vanbergen_marielouise_1842` born **Waasmunster**, not Hamme; `vermandel_ida_1825` born **Zelzate**.
...
- **AGATHA is not logged in.** Act pages render by direct id, but images show *"Gelieve in te
  loggen"*. That is the one thing standing between `vannieuwenhuyse_petrus_1649` and `doc`: his
...
both `vanbergen_marielouise_1842`, both clearing 23 bits on *surname + birth-year ±1 + father's
...
| pass | graft | identifier 1 | identifier 2 |
|---|---|---|---|
| 1 | 8 children of `vincke_lucien_1840`, + Florence Amelie and 4 Van Iseghem children — 12 people | father's full forename pair **and** mother's rare full name on each Diksmuide act | two independent filters (by father-name, by mother-surname) return the identical set — no rival couple |
| 2 | 2 Bossin children + 5 siblings of `bossin_ludovicus_1879` — 10 people | each parent's **age-implied birth year matching the held date to the day**, plus birthplace | the couple's own 1858 marriage act as anchor: the mother's stated age matches exactly in **5 of 5** acts |
...
| 8 | 7 siblings of `vaniseghem_eduardus_1880` — 7 people | the **parent pair**, both names in full — because "Joannes Vaniseghem" alone matches **nine different men** in this index | 6 of 7 cross-locked by a day-level date from a separately transcribed death or marriage act |
...
- **`huyghebaert_appolonia_1830`'s best candidate** (`graftable: True`) — a population-register row for *Marie* Huyghebaert, 80 km away. **`vincke_lucien_1840`'s best** — a Brussels act whose matched pid was a *witness*, Gustave. **`thumas_georges_1843`'s best at 39 bits** — his own infant brother's death act, a person already in the tree.
...
- **`vincke_emma_1880`'s `doc` upgrade** — AGATHA's West-Flanders birth project publishes no scan link at all; the reference "SCAN 392 GSU" is bare text with no IIIF or viewer anywhere in the DOM.
...
`vanalderweireldt_antoine_1780`'s death lost its day-level precision entirely — `1808-07-21` was a registration
date, and three sources give three different days. `wyllie_brigitte_1779`'s birth was reduced to the
...
One re-authenticated session would settle: `vincke_emma_1880`'s birth act image, the Zaventem scan
...
3. **The cover credits the wrong sibling** — it claimed an act resolved `peremans_maria`, whose name does not appear in it. Siblings share held parent identifiers, and it appears to credit any of them once the pair matches, without checking the participant's own forename.
