---
id: thumas_jean_1845
name: Jean Baptiste Zénon Thumas
surname: Thumas
birth:
  date: "1845"
  raw: declared at Grez-Doiceau on 20 August 1845
  place: Grez-Doiceau
confidence: sup
line: thumas
father:
  id: thumas_georges_1804
  confidence: sup
mother:
  id: joostens_marie_1810
  confidence: sup
siblings:
  - id: thumas_charles_1850
    confidence: sup
  - id: thumas_georges_1836
    confidence: sup
  - id: thumas_georges_1843
    confidence: sup
  - id: thumas_marie_1835
    confidence: sup
sources:
  - S25
  - netradyle-grez-naissances
---

Younger brother of [[thumas_georges_1836]]. His birth was declared at Grez-Doiceau on **20 August
1845** (S25), between his brother [[thumas_georges_1843]] in 1843 and [[thumas_charles_1850]] in 1850.

`raw` rather than a date: the index gives the declaration, not the birth.

`sup`: an index row, no register image read.

FRONTIER: his birth act, and whether he lived to marry — nothing is known of him after 1845.
*Zénon* is an unusual enough forename to search on its own, which is worth trying before the
common ones.

CORROBORATED, INDEX-LEVEL (2026-07-27): [[netradyle-grez-naissances]] independently
indexes him as **19 August 1845** — one day off the 20 August declaration held here (S25),
the same order of drift documented across this venue — with father Georges and mother
`JOSTENS Marie Catherine`. Two independent index compilers now agree on the rare tripartite
forename and both parents. The same row gives her origin commune as Woluwe-Saint-Pierre,
which conflicts with the Woluwe-Saint-Lambert on [[joostens_marie_1810]]'s own death act; the
disagreement is recorded there, not resolved here.
